...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.MainWindow.resize(360, 375)
        self.MainWindow.setMinimumSize(QtCore.QSize(360, 375))
        self.MainWindow.setMaximumSize(QtCore.QSize(360, 375))
        icon = QtGui.QIcon()
...
"color: rgb(255, 255, 255);")
        ...
        self.pushButton_2.setGeometry(QtCore.QRect(70, 310, 221, 41))
        self.pushButton_2.setStyleSheet("background-color: rgb(7, 85, 240);\n"
                                 "color: rgb(255, 255, 255);")
...
"background-image: url(image/loginicon.jpg);")
    ...
    def register(self):
        ...
        if self.user=='':
            QMessageBox.information(self.MainWindow,'提示','注册QQ账号不能为空!', QMessageBox.Ok | QMessageBox.Close, QMessageBox.Close)
        elif user_l<5 or user_l>10:
            QMessageBox.information(self.MainWindow, '提示', '注册QQ账号长度不对!', QMessageBox.Ok | QMessageBox.Close, QMessageBox.Close)
        else:
            if password=='':
                QMessageBox.information(self.MainWindow, '提示', '注册密码不能为空!', QMessageBox.Ok | QMessageBox.Close, QMessageBox.Close)
            else:
    ...
    def register_recv(self):
        ...
        print(recv_info)
        if str(recv_info)=='true':
            QMessageBox.information(self.MainWindow, '注册成功', '注册成功!', QMessageBox.Ok | QMessageBox.Close, QMessageBox.Close)
            #QtCore.QCoreApplication.instance().quit()
            # widget.hide()
            # widget1.show()
            # ui1.label.setText(self.user)
        elif str(recv_info)=='false—no':
            QMessageBox.information(self.MainWindow, '失败', '注册失败，无此账号!!', QMessageBox.Ok | QMessageBox.Close,QMessageBox.Close)
        elif str(recv_info)=='false-is':
            QMessageBox.information(self.MainWindow, '失败', '此账号已存在!', QMessageBox.Ok | QMessageBox.Close,QMessageBox.Close)
